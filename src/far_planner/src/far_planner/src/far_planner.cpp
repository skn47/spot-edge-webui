...
void FARMaster::Init() {
  ...
  waypoint_sub_       = nh_->create_subscription<geometry_msgs::msg::PoseStamped>("/goal_pose", 1, std::bind(&FARMaster::WaypointCallBack, this, std::placeholders::_1));
  ...
  path_pub_           = nh_->create_publisher<nav_msgs::msg::Path>("/far_path", 5);
...
void FARMaster::PlanningCallBack() {
  ...
  if (goal_ptr == NULL) {
    ...
    if (graph_planner_.PathToGoal(goal_ptr, global_path, nav_node_ptr_, current_free_goal, is_planning_fails, is_reach_goal, is_current_free_nav) && nav_node_ptr_ != NULL) {
      ...
      // Publish interpolated path for pure pursuit controller
      nav_msgs::msg::Path path_msg;
      path_msg.header.frame_id = master_params_.world_frame;
      path_msg.header.stamp = nh_->now();
      
      // Interpolate dense path from robot to waypoint
      const double path_spacing = 0.1;  // 10cm resolution for smooth pure pursuit
      Point3D diff = waypoint - robot_pos_;
      double dist = diff.norm();
      int num_points = std::max(2, static_cast<int>(std::ceil(dist / path_spacing)));
      
      for (int i = 0; i < num_points; i++) {
          float t = static_cast<float>(i) / (num_points - 1);
          Point3D interp_p = robot_pos_ + diff * t;
          
          geometry_msgs::msg::PoseStamped pose_stamped;
          pose_stamped.header = path_msg.header;
          pose_stamped.pose.position = FARUtil::Point3DToGeoMsgPoint(interp_p);
      ...
      empty_path.header.frame_id = master_params_.world_frame;
      empty_path.header.stamp = nh_->now();
...
void FARMaster::WaypointCallBack(const geometry_msgs::msg::PoseStamped::SharedPtr route_goal) {
  if (!is_graph_init_) {
